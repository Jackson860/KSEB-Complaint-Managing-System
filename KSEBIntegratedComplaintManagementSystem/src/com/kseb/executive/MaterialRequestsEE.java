...
public class MaterialRequestsEE extends HttpServlet{
	...
	try {
		...
		out.print("<table>");
		out.print("<tr><th>Complaint ID</th><th>Complaint Details</th><th>Lineman ID</th><th>Request ID</th><th>Materials Requested</th><th>Approve Request</th></tr>");
		...
		while(rs.next()) {
			...
			while(rsNew.next()) {
				...
				rsNewx.close();
				out.print("<tr><td>"+complaintID+"</td><td>"+complaintDetails+"</td><td>"+linemanID+"</td><td>"+requestID+"</td><td>"+materialDetails+"</td><td>Approve</td></tr>");
			}
			rsNew.close();
